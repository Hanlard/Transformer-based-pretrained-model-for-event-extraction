...
class Net(nn.Module):
    ...
    def predict_triggers(self, tokens_x_2d, entities_x_3d, postags_x_2d, head_indexes_2d, triggers_y_2d, arguments_2d,
        ...
        if self.training:
            ...
            ## bert 最后一层
            x_1 = encoded_layers[-1]
            # x_2 = encoded_layers[-2]
        else:
            self.bert.eval()
            with torch.no_grad():
                encoded_layers, _ = self.bert(tokens_x_2d)
    ...
    def predict_arguments(self, sen_mask_id, argument_keys, arguments_2d, adjm):
        ...
        ######### argument CRF #########
        argument_logits = self.arg_fc1(argument_hidden)
        arguments_y_2d = torch.LongTensor(arguments_y_2d).to(self.device)
        mask = torch.stack(mask).to(self.device)


        argument_loss = self.arg_CRF.neg_log_likelihood_loss(feats=argument_logits, mask= mask, tags=arguments_y_2d)
        _, argument_hat_1d = self.arg_CRF.forward(feats=argument_logits, mask=mask)
        ###############################
