"""
2020/2/3修改
在argument_key中加入了错误预测的触发词，对应的argument全是O
在计算CRF的logits时加入了tanh激活函数
"""
import os
import torch
...
class Net(nn.Module):
    def __init__(self, trigger_size=None, entity_size=None, all_postags=None, PreModel=None, hyper_para =None, idx2trigger = None, postag_embedding_dim=50,
        ...
        self.arg_fc1 = nn.Sequential(
            nn.Dropout(0.5),
            nn.Linear(self.hidden_size*2+self.tri_embsize+self.pos_embsize, argument_size + 2, bias=True),
        ...
        ## fc_trigger 将触发词的BERT表达，变换维度
        # self.fc_trigger = nn.Linear(in_features=self.hidden_size, out_features=self.tri_embsize).to(
        #     device=device)
    ...
    def predict_triggers(self, tokens_x_2d, entities_x_3d, postags_x_2d, head_indexes_2d, triggers_y_2d, arguments_2d,
        ...
        ## 根据触发词结果进行嵌入
        trigger_crf1_emb = self.trigger_embed(trigger_hat_2d)

        ## 将触发词嵌入加入BERT表达t         ## 将触发词嵌入加入BERT表达t
        ## [batch_size, SEQ_LEN , size+50]
        self.emb = torch.cat([emb, trigger_crf1_emb],dim=-1) # hidden_size + trigger_embsize
    ...
    def predict_arguments(self, sen_mask_id, argument_keys, arguments_2d, adjm):
        ...
        for k,trigger in enumerate(argument_keys):
            ...
            trigger_BERTemb = torch.unsqueeze(trigger_BERTemb,dim=0).repeat(SEQ_LEN, 1)  # [SEQ_LEN,tri_embsize]
            argument_hidden.append(torch.cat([x[i],trigger_BERTemb],dim=-1))    # [SEQ_LEN,hidden_size+tri_embsize*2]
        ...
        argument_logits1 = nn.functional.leaky_relu_(argument_logits1)
        arguments_y_2d = torch.LongTensor(arguments_y_2d).to(self.device)
        mask = torch.stack(mask).to(self.device)
        argument_loss = self.arg_CRF1.neg_log_likelihood_loss(feats=argument_logits1, mask= mask, tags=arguments_y_2d)
        _, argument_hat_1d = self.arg_CRF1.forward(feats=argument_logits1, mask=mask)
        ###############################
